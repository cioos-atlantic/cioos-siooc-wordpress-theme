<?php
/**
 * ThinkUpThemes.com Upgrade notice - Experon Theme
 * 
...
function thinkup_latest_theme_version($interval) {
	...
	$experon_notifier_file_url = 'http://thinkupcloud.com/cloudfiles/thinkupthemes/updates/themes/update_' . str_replace( ' pro', '', strtolower( $theme_data['Name'] ) ) . '.xml'; // Set to ThinkUpCloud - replace with CDN source in future.

	// Check if Dropbox link is broken
	if( function_exists('curl_init') ) { // if cURL is available, use it...
		$dropbox_ch = curl_init($experon_notifier_file_url);
		...
		$dropbox_check = @file_get_contents($experon_notifier_file_url); // ...if not, use the common file_get_contents()
	}

	// If dropbox is empty then check thinkupcloud.com
	if( empty( $dropbox_check ) ) {
		$experon_notifier_file_url = 'http://thinkupcloud.com/cloudfiles/thinkupthemes/updates/themes/update_' . str_replace( ' pro', '', strtolower( $theme_data['Name'] ) ) . '.xml';
	}

	$experon_db_cache_field = 'experon_contempo-notifier-cache';
	$experon_db_cache_field_last_updated = 'experon_contempo-notifier-last-updated';
	$experon_last = get_option( $experon_db_cache_field_last_updated );
	$experon_now = time();
	// check the cache
	if ( !$experon_last || (( $experon_now - $experon_last ) > $interval) ) {
		// cache doesn't exist, or is old, so refresh it
		if( function_exists('curl_init') ) { // if cURL is available, use it...
			$experon_ch = curl_init($experon_notifier_file_url);
			curl_setopt($experon_ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($experon_ch, CURLOPT_HEADER, 0);
			curl_setopt($experon_ch, CURLOPT_TIMEOUT, 10);
			$experon_cache = curl_exec($experon_ch);
			curl_close($experon_ch);
		} else {
			$experon_cache = @file_get_contents($experon_notifier_file_url); // ...if not, use the common file_get_contents()
		}
		
		if ($experon_cache) {			
			// we got good results
			update_option( $experon_db_cache_field, $experon_cache );
			update_option( $experon_db_cache_field_last_updated, time() );			
		}
		// read from the cache file
		$experon_notifier_data = get_option( $experon_db_cache_field );
	}
	else {
	...
	if ( strpos( $experon_notifier_data,'looking for. Check out our' ) !== false) {
	} else {
		@$experon_xml = simplexml_load_string($experon_notifier_data); 
	}
	return $experon_xml;
}
