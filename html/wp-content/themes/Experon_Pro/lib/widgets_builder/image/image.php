...
class thinkup_builder_imagetheme extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array( 'classname' => 'thinkup_builder_imagetheme', 'description' => __( 'Add an image from the media library.', 'experon' ) );
		$control_ops = array( 'id_base' => 'thinkup_builder_imagetheme' );
		parent::__construct('thinkup_builder_imagetheme', __('Image', 'experon'), $widget_ops, $control_ops);
	...
	function admin_setup() {
		...
		wp_localize_script( 'thinkup-builder-image', 'ThinkupBuilderImage', array(
			'frame_title' => __( 'Select an Image', 'experon' ),
			'button_title' => __( 'Insert Into Widget', 'experon' ),
		) );
	}
...
//		$instance       = wp_parse_args( (array) $instance, self::get_defaults() );
		...
		echo '<p style="margin-bottom: 20px;"><label for="' . $this->get_field_id('title') . '" style="display: inline-block;width: 100px;" >' . __('Widget Title', 'experon') . ':</label>',
		...
		echo '<div class="uploader">',
			 '<input type="submit" class="button" name="' . $this->get_field_name('uploader_button') . '" id="' . $this->get_field_id('uploader_button') . '" value="' . __('Select an Image', 'experon') . '" onclick="imageWidget.uploader( &#39;' . $id_prefix . '&#39;, &#39;' . $id_prefix . '&#39; ); return false;" />',
		...
		echo '<div id="' . $this->get_field_id('custom_size_selector') . '" class="image-image-size" >',
			 '<p style="margin-bottom: 20px;"><label for="' . $this->get_field_id('size') . '" style="display: inline-block;width: 150px;">' . __('Size', 'experon') . ':</label>',
			 '<select name="' . $this->get_field_name('size') . '" id="' . $this->get_field_id('size') . '" onChange="imageWidget.toggleSizes( &#39;' . $id_prefix . '&#39;, &#39;' . $id_prefix . '&#39; );" style="display: inline-block;width: 200px;margin: 0;">';
					...
					$possible_sizes = apply_filters( 'image_size_names_choose', array(
						'full'      => __('Full Size', 'experon'),
						'thumbnail' => __('Thumbnail', 'experon'),
						'medium'    => __('Medium', 'experon'),
						'large'     => __('Large', 'experon'),
					) );
	//				$possible_sizes[self::CUSTOM_IMAGE_SIZE_SLUG] = __('Custom', 'experon');
		...
		echo '<p><label for="' . $this->get_field_id('link') . '" style="display: inline-block;width: 150px;" >' . __('Link', 'experon') . ':</label>',
