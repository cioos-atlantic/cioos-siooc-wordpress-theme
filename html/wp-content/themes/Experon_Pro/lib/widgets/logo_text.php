...
if ( ! class_exists( 'thinkup_widget_logotext' ) ) {
	class thinkup_widget_logotext extends WP_Widget {
		...
		function widget($args, $instance) {		
			...
			$logo = empty($instance['logo']) ? __( 'Tags', 'experon' ) : apply_filters('widget_title', $instance['logo']);
			$text = $instance['text'];
	...
	add_action( 'widgets_init', function() { return register_widget( "thinkup_widget_logotext" ); } );
}