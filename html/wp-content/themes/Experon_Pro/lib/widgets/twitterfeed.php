...
if ( ! class_exists( 'thinkup_widget_tweet_scroll' ) ) {
	class thinkup_widget_tweet_scroll extends WP_Widget {
		...
		function form($instance) {
			...
			<p>
				<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'experon') ?></label>
			...
			<!-- Username: Text Input -->
			<p>
				<label for="<?php echo $this->get_field_id('username'); ?>"><?php _e('Twitter Username:', 'experon') ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('visible_tweets'); ?>"><?php _e('Number of tweets to show:', 'experon') ?></label>
			...
			<!-- Time: Checkbox Input -->
			<p>
					<label for="<?php echo $this->get_field_id('time'); ?>"><?php _e('Show or hide timestamp:', 'experon') ?></label><br />
				<input type="checkbox" id="<?php echo $this->get_field_id('time'); ?>" name="<?php echo $this->get_field_name('time'); ?>" <?php if ($instance['time'] == true) echo 'checked' ?>/><label for="<?php echo $this->get_field_id('time'); ?>"> <?php _e('Show', 'experon') ?></label>
			</p>

			<!-- Date Format: Radio Input -->
			<p>
				<label for="<?php echo $this->get_field_id('date_format'); ?>"><?php _e('Date Format:', 'experon') ?></label><br />
				<input type="radio" id="<?php echo $this->get_field_id('date_format'); ?>-1" name="<?php echo $this->get_field_name('date_format'); ?>" value="style1" <?php if ($instance['date_format'] == 'style1') echo 'checked=checked' ?>/> <label for="<?php echo $this->get_field_id('date_format'); ?>-1"> <?php _e('European', 'experon') ?></label><br />
				<input type="radio" id="<?php echo $this->get_field_id('date_format'); ?>-2" name="<?php echo $this->get_field_name('date_format'); ?>" value="style2" <?php if ($instance['date_format'] == 'style2') echo 'checked=checked' ?>/> <label for="<?php echo $this->get_field_id('date_format'); ?>-2"> <?php _e('American', 'experon') ?></label>
			</p>

			<!-- Consumer Key -->
			<p>
				<label for="<?php echo $this->get_field_id('consumer_key'); ?>"><?php _e('Consumer Key:', 'experon') ?></label>
			...
			<!-- Consumer Secret -->
			<p>
				<label for="<?php echo $this->get_field_id('consumer_secret'); ?>"><?php _e('Consumer Secret:', 'experon') ?></label>
			...
			<!-- Access Token -->
			<p>
				<label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('Access Token:', 'experon') ?></label>
			...
			<!-- Access Token Secret -->
			<p>
				<label for="<?php echo $this->get_field_id('access_token_secret'); ?>"><?php _e('Access Token Secret:', 'experon') ?></label>
	...
	add_action( 'widgets_init', function() { return register_widget( "thinkup_widget_tweet_scroll" ); } );
