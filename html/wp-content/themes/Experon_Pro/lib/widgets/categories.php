...
if ( ! class_exists( 'thinkup_widget_categories' ) ) {
	class thinkup_widget_categories extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			echo $before_widget;
			$title = empty($instance['title']) ? __( 'Categories', 'experon' ) : apply_filters('widget_title', $instance['title']);
			if (!empty($title))
					...
					echo '<ul>';
						foreach( $terms as $term ) {
						  echo '<li class="cat-item cat-item-24">',
								'<a href="' . get_category_link( $term->term_id ) . '" title="' . sprintf( __( 'View all posts in %s', 'experon' ), $term->name ) . '" ' . '><span>' . $term->name;
									if ($instance['postswitch'] == 'on') {
										echo '  (' . $term->count.')';
									}
						  echo 	'</span></a></li> ';
	...
	add_action( 'widgets_init', function() { return register_widget( "thinkup_widget_categories" ); } );
}