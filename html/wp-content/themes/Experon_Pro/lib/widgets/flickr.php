...
if ( ! class_exists( 'thinkup_widget_flickr' ) ) {
	class thinkup_widget_flickr extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			echo $before_widget;
			$title = empty($instance['title']) ? __( 'Flickr Feed', 'experon') : apply_filters('widget_title', $instance['title']);
			if (!empty($title))
			...
			if($flickrphoto->photos->photo) {
				...
				echo '<div class="flickr-photo">'.
					 ...
					 '<img src="' . $flickrphoto_url . '" alt="' . $flickrphoto['title'] . '" width="54" height="54" />',
					 '<div class="image-overlay"></div>',
					 '</a>',
					 '</div>';
				}
			} else {
				echo '<div class="error-icon">' . __( 'Oops! There&#39;s been an error!', 'experon') . '<br />' .  __( 'Check that the Flickr ID is correct at', 'experon') . '<a href="http://idgettr.com/">http://idgettr.com/</a>.</div>';
				}
	...
	add_action( 'widgets_init', function() { return register_widget( "thinkup_widget_flickr" ); } );
}