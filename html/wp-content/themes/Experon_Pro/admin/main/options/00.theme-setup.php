...
function thinkup_title_select() {
	...
	if ( is_page() ) {
		...
		printf( '<span>' . __( 'Category Archives: ', 'grow' ) . '</span>' . '%s', single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		printf( '<span>' . __( 'Tag Archives: ', 'grow' ) . '</span>' . '%s', single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		the_post();
		printf( '<span>' . __( 'Author Archives: ', 'grow' ) . '</span>' . '%s', get_the_author() );
		rewind_posts();
	} elseif ( is_day() ) {
		printf( '<span>' . __( 'Daily Archives: ', 'grow' ) . '</span>' . '%s', get_the_date() );
	} elseif ( is_month() ) {
		printf( '<span>' . __( 'Monthly Archives: ', 'grow' ) . '</span>' . '%s', get_the_date( 'F Y' ) );
	} elseif ( is_year() ) {
		printf( '<span>' . __( 'Yearly Archives: ', 'grow' ) . '</span>' . '%s', get_the_date( 'Y' ) );
	} elseif ( is_post_type_archive( 'portfolio' ) ) {
		printf( '<span>' . __( 'Portfolio', 'grow' ) . '</span>' );
	} elseif ( is_post_type_archive( 'client' ) ) {
		printf( '<span>' . __( 'Our Clients', 'grow' ) . '</span>' );
	} elseif ( is_post_type_archive( 'team' ) ) {
		printf( '<span>' . __( 'Our Team', 'grow' ) . '</span>' );
	} elseif ( is_post_type_archive( 'testimonial' ) ) {
		printf( '<span>' . __( 'Customer Testimonials', 'grow' ) . '</span>' );
...
global $thinkup_general_breadcrumbdelimeter;
	...
	if ( ! is_front_page() ) {
		...
		if ( is_single() ) {
			...
			$output .= '<span class="breadcrumbs-cat">' . __( 'Archive Category: ', 'experon' ) . '</span>' . esc_html( single_cat_title("", false) );
		} elseif ( is_tag() ) {
			$output .= '<span class="breadcrumbs-tag">' . __( 'Posts Tagged: ', 'experon' ) . '</span>' . single_tag_title("", false);
		} elseif ( is_day()) {
			...
			$output .= __( 'Archived Article(s) by Author: ', 'experon' ) . esc_html( $user_info->display_name );
		} elseif ( is_404() ) {
			$output .= __( 'Error 404 - Not Found.', 'experon' );
		} elseif ( is_post_type_archive( 'portfolio' )	) {
			$output .= __( 'Portfolio', 'experon' );
		} elseif ( is_post_type_archive( 'client' )	) {
			$output .= __( 'Clients', 'experon' );
		} elseif ( is_post_type_archive( 'team' )	) {
			$output .= __( 'Team', 'experon' );
		} elseif ( is_post_type_archive( 'testimonial' )	) {
			$output .= __( 'Testimonials', 'experon' );
		} elseif ( is_post_type_archive( 'product' ) and function_exists( 'thinkup_woo_titleshop_archive' ) ) {
...
if ( ! function_exists( 'thinkup_input_showimagesizes' ) ) {
	function thinkup_input_showimagesizes($sizes) {
		...
		// 1 Column Layout
		$sizes['column1-1/2'] = __( 'Full - 1:2', 'experon' );
		$sizes['column1-1/3'] = __( 'Full - 1:3', 'experon' );
		$sizes['column1-1/4'] = __( 'Full - 1:4', 'experon' );
		$sizes['column1-2/5'] = __( 'Full - 2:5', 'experon' );

		// 2 Column Layout
		$sizes['column2-1/1'] = __( 'Half - 1:1', 'experon' );
		$sizes['column2-1/2'] = __( 'Half - 1:2', 'experon' );
		$sizes['column2-2/3'] = __( 'Half - 2:3', 'experon' );
		$sizes['column2-3/5'] = __( 'Half - 3:5', 'experon' );

		// 3 Column Layout
		$sizes['column3-1/1'] = __( 'Third - 1:1', 'experon' );
		$sizes['column3-2/5'] = __( 'Third - 2:5', 'experon' );
		$sizes['column3-2/3'] = __( 'Third - 2:3', 'experon' );
		$sizes['column3-3/4'] = __( 'Third - 3:4', 'experon' );

		// 4 Column Layout
		$sizes['column4-1/1'] = __( 'Quarter - 1:1', 'experon' );
		$sizes['column4-2/3'] = __( 'Quarter - 2:3', 'experon' );
		$sizes['column4-3/4'] = __( 'Quarter - 3:4', 'experon' );
...
function thinkup_input_commentspopuplink( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
    ...
    if ( false === $zero ) $zero = __( 'No Comments','experon' );
    if ( false === $one ) $one = __( '1 Comment','experon' );
    if ( false === $more ) $more = __( '% Comments','experon' );
    if ( false === $none ) $none = __( 'Comments Off','experon' );
    ...
    if ( post_password_required() ) {
        $str = __('Enter your password to view comments.','experon');
        return $str;
    }
    ...
    $str .= ' title="' . esc_attr( sprintf( __('Comment on %s','experon'), $title ) ) . '">';
...
function thinkup_comments_returnstring( $zero = false, $one = false, $more = false, $deprecated = '' ) {
    ...
    if ( $number > 1 )
        $output = str_replace('%', number_format_i18n($number), ( false === $more ) ? __( '% Comments', 'experon' ) : $more);
    elseif ( $number == 0 )
        $output = ( false === $zero ) ? __( 'No Comments', 'experon' ) : $zero;
    else // must be one
        $output = ( false === $one ) ? __( '1 Comment', 'experon' ) : $one;
