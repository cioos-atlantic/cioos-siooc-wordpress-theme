...
if ( ! class_exists( 'thinkup_widget_popularposts' ) ) {
	class thinkup_widget_popularposts extends WP_Widget {
		...
		$comment_input = NULL;
			...
			echo $before_widget;
			$title = empty($instance['title']) ? __( 'Popular Posts', 'experon' ) : apply_filters('widget_title', $instance['title']);
			if (!empty($title))
			...
			while ($posts->have_posts()) : $posts->the_post();
				...
				if ( $instance['comments'] == 'on' ) {
					$comment_input = thinkup_input_commentspopuplink( __( 'No Comments', 'experon' ), __( '1 Comment', 'experon' ), __( '% Comments', 'experon' ), 'comment' );
				}			
	...
	add_action( 'widgets_init', function() { return register_widget( "thinkup_widget_popularposts" ); } );
}