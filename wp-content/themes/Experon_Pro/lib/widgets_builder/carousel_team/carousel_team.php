...
class thinkup_builder_carouselteam extends WP_Widget {
	...
	function form($instance) {
		...
		echo '<p><label for="' . $this->get_field_id('heading') . '" style="display: inline-block;width: 150px;">Carousel Heading:</label><input class="widefat" id="' . $this->get_field_id('heading') . '" name="' . $this->get_field_name('heading') . '" type="text" value="' . esc_attr($heading) . '" style="display: inline-block;width: 200px;margin: 0;" /></p>';
		...
		echo '<p><label for="' . $this->get_field_id('link_style') . '" style="display: inline-block;width: 150px;" >Link Style:</label>
			<select name="' . $this->get_field_name('link_style') . '" id="' . $this->get_field_id('link_style') . '" style="display: inline-block;width: 200px;margin: 0;" >
			<option '; ?><?php if($link_style == "style1") { echo "selected"; } ?><?php echo ' value="style1">Style 1</option>
			<option '; ?><?php if($link_style == "style2") { echo "selected"; } ?><?php echo ' value="style2">Style 2</option>
			</select>
		</p>';

		echo '<p><label for="' . $this->get_field_id('show_name') . '">Show Member Name?</label>&nbsp;<input id="' . $this->get_field_id('show_name') . '" name="' . $this->get_field_name('show_name') . '" type="checkbox" ' . $show_name_check . ' style="margin-left: 18px;" /></p>';
	...
	function update($new_instance, $old_instance) {
		...
		$instance['link_style']     = $new_instance['link_style'];
	...
	function widget($args, $instance) {
		...
		echo do_shortcode( '[carousel_team items="' . $items .'" scroll="' . $scroll . '" speed="' . $speed . '" effect="' . $effect . '" show_link="' . $show_link . '" link_style="' . $link_style . '" show_name="' . $show_name . '" show_position="' . $show_position . '" show_excerpt="' . $show_excerpt . '" show_social="' . $show_social . '" tag="' . $tag . '" heading="' . $heading . '"]' );
...
add_action( 'widgets_init', function() { return register_widget( "thinkup_builder_carouselteam" ); } );
