...
class thinkup_builder_carouselportfoliotheme extends WP_Widget {
	...
	function form($instance) {
		...
		echo '<p><label for="' . $this->get_field_id('link_style') . '" style="display: inline-block;width: 150px;" >Links Style:</label>
	...
	function widget($args, $instance) {
		...
		echo do_shortcode( '[carousel_portfolio heading="' . $heading . '" items="' . $items .'" scroll="' . $scroll . '" speed="' . $speed . '" effect="' . $effect . '" link_icon="' . $link_icon . '" lightbox_icon="' . $lightbox_icon . '" links_style="' . $link_style . '" title="' . $toggle_title . '" details="' . $toggle_details . '" content_style="' . $content_style . '" category="' . $category . '"]' );

		echo '</div>';
	}
}
add_action( 'widgets_init', function() { return register_widget( "thinkup_builder_carouselportfoliotheme" ); } );
