...
class thinkup_woo_WpMenuCart {
	...
	public function wpmenucart_menu_item() {
		...
		//use regular WP i18n
		$viewing_cart = __('View your shopping cart', 'experon');
		$start_shopping = __('Start shopping', 'experon');
		$cart_contents = sprintf(_n('<span class="woo-cart-count-before">Cart (</span>%d<span class="woo-cart-count-after"> Item)</span>', '<span class="woo-cart-count-before">Cart (</span>%d<span class="woo-cart-count-after"> Items)</span>', $item_data['cart_contents_count'], 'experon'), $item_data['cart_contents_count']);
	...
	public function wpmenucart_menu_drop() {
		...
		if ($item_data['cart_contents_count'] == 0) {
			$menu_drop_content = '';
			$menu_drop_content .= '<div class="woo-cart-menu-dropdown" style="display: none;">' . __('Cart: ', 'experon' ) . ' <span class="amount">'.$item_data['cart_total'].'</span></div>';
